...
class RSG(nn.Module):
    def __init__(self, n_center = 3, feature_maps_shape = [32, 16, 16], num_classes=10, contrastive_module_dim = 128, head_class_lists = [], transfer_strength = 1.0, epoch_thresh = 100):
        ...
        self.linear = nn.Parameter(torch.randn(num_classes, self.C, n_center).to(self.device))
        self.bias = nn.Parameter(torch.ones(num_classes, n_center).to(self.device)) 

        self.centers = nn.Parameter(torch.zeros(num_classes, n_center, self.C).to(self.device))
    ...
    def compute_cesc_loss(self, feature_maps, c, gamma, target, epoch):
        ...
        if epoch <= self.epoch_thresh:
            ...
            self.contrastive_module = self.contrastive_module.to(self.device)
            self.contrastive_fc = self.contrastive_fc.to(self.device)
    ...
    def compute_mv_loss(self, origin_feature, origin_center, target_center, target_features, gamma_head, target, gamma_tail):
        ...
        index =  self.to_one_hot_vector(self.n_center, index).unsqueeze(1).to(self.device)
        index_ = self.to_one_hot_vector(self.n_center, index_).unsqueeze(1).to(self.device)

        c_o = torch.bmm(index, c.view(-1, self.n_center, self.H*self.W*self.C).double()).view(ori_f.size()).to(self.device)
        c_t = torch.bmm(index_, c_.view(-1, self.n_center, self.H*self.W*self.C).double()).view(ori_f.size()).to(self.device)

        var_map = (ori_f - c_o.float()).to(self.device)
        self.vec_transformation_module = self.vec_transformation_module.to(self.device)
        ...
        loss = F.cross_entropy(pair_pred, torch.zeros(num).long().to(self.device)) + \
        (torch.sum(torch.abs(torch.norm(var_map_t.view(-1,self.C), dim=1) - torch.norm(var_map.view(-1, self.C), dim=1)))  + torch.sum(torch.abs(target_features_norm * var_map_norm - torch.ones(target_features_norm.size()).to(self.device))))/(num)
    ...
    def forward(self, feature_maps, head_class_lists, target, epoch):
        ...
        head_class_lists_tensor = torch.Tensor(head_class_lists).to(self.device)
        ...
        index_head = torch.sum((target_expand == head_class_lists_tensor).long(), dim = 1).to(self.device)
        index_tail = 1 - index_head
        index_head_ = torch.eq(index_head, 1).to(self.device)
        index_tail_ = torch.eq(index_tail, 1).to(self.device)
        ...
        loss_mv_total = torch.zeros(loss_cesc.size()).to(self.device)
