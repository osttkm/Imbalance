...
def furthest_first(unlabeled_embeddings, labeled_embeddings, badget, init_num,seed):
    ...
    for i in range(init_num):
        ...
        if i==0:
            best_idxs = idxs
        if (abs(org_var-torch.var(unlabeled_embeddings[best_idxs])) > abs(org_var-iner_dis)) and (len(idxs)==len(np.unique(idxs))):
            best_idxs = idxs
            assert len(best_idxs)==len(np.unique(best_idxs))
